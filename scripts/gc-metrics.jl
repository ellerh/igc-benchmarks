...
parse_telemetry_line(line)::Event =
  let (tsc, name, args) = match(telemetry_line_rx, line).captures
    Event(parse(UInt, tsc, base = 16) / tsc_frequency, Symbol(name), args)
  end
      ...
      let e = g.events[i]
        if e.name == end_name
          ...
          if is_no_work_was_done_event(e)
            iterate(g, (i+1, nothing, nothing))
          else
            (GCPause(start, e.time - start), (i+1, nothing, nothing))
          end
        elseif haskey(event_pairs, e.name)
          @assert isnothing(start) "begin without end"
          iterate(g, (i+1, e.time, event_pairs[e.name]))
        else
          iterate(g, (i+1, start, end_name))
        end
      end
    end
  end
...
pmu(duration, p, trace, resolution) =
  let mus = [
      mutator_utilization(trace, start, duration) for
      start in range(0, trace.duration-duration, length = resolution)
    ]
...
plot_pmu(trace; xres = 500, sres = 5000) =
  let xs = logrange(0.005, trace.duration, length = xres)
    ...
    Plots.plot(
      ...
      xguide = "Time slice (sec)",
      yguide = "PMU (1)",
      title = "PMU curve for range of percentiles for " * trace.label;
      subplot_kwargs...,
    )
  end
