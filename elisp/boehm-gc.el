;;; boehm-gc.el -- benchmark testing GC performance   -*- lexical-binding: t -*- 
;;
...
(cl-defstruct node left right dummy1 dummy2)
...
(defun gcbench (stretch-tree-depth)
  ...
  (let* ((kLongLivedTreeDepth (- stretch-tree-depth 2))
    ...
    ;; (format t "Creating a binary tree of depth ~d~%" kLongLivedTreeDepth)
    (let ((longLivedTree (make-node)))
      ...
      (let ((array (make-vector kArraySize 0.0)))
        (cl-loop for i below (floor kArraySize 2)
		 do (setf (aref array i) (/ 1.0 (1+ i))))

        (cl-do ((d kMinTreeDepth (+ d 2)))
            ((> d kMaxTreeDepth))
          (let ((iteration-count (iteration-count d)))
            ...
            (dotimes (_i iteration-count) (populate d (make-node)))
            ;; (format t "GCBench: Bottom up construction~%")
            (dotimes (_i iteration-count) (make-tree d))))

        ;; these are fake references to LongLivedTree and array to
        ;; keep them from being optimized away
        (cl-assert (not (null longLivedTree)))
        (cl-assert (let ((n (min 1000 (1- (floor (length array) 2)))))
                     (= (round (aref array n)) (round (/ 1.0 (1+ n))))))))))

(defun gc-benchmark (&optional k)
  ;; (format t "The GC should touch about ~d megabytes of heap storage.~%"
  ;;         (expt 2 (- k 13)))
